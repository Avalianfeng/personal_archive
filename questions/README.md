...
```text
01-Library → 整理 Agent(s) 并行 → 05-Imports/01-pending/*.jsonl（一来源一文件）
                                    ↓
                         主 Agent：ingest --dry-run → manage accept --json（单点）
                                    ↓
                         04-Store + 03-Meta/03-generated/ + 02-Views（sync）
                                    ↓
                         主 Agent：回写卡片 + Library/README.md
```

**多 Agent**：子 Agent 只写 jsonl + 本来源卡片；accept / README / 派生物由主 Agent 串行。见 [01-整理Agent启动.md §多 Agent 协作](./03-规则与审计-Meta/04-prompts-Agent提示/01-整理Agent启动.md#多-agent-协作并行整理--串行-accept)。
