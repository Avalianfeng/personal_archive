...
**文件生命周期** — 与题目 `status` 无关。见 [02-schema-格式契约/01-格式规范.md](../../03-规则与审计-Meta/02-schema-格式契约/格式规范.md)。
...
1. 从 `02-问题地图-Views/` 或 `bank/` **复制**（非剪切）到本目录
...
| reason | 说明 |
| --- | --- |
| `duplicate` | 与 02-问题地图-Views/bank 中已有题**完全重复** |
| `leading` | 引导性过强 |
| `low_density` | 信息密度低 |
