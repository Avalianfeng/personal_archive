...
从 [categories/](../categories/README.md) 人工或 Agent 筛选后的**高质量子集**。现阶段可为空。

## 与 categories 的关系

```text
categories/   全量分类地图（可能含重复、低质量、待改写题）
     ↓ 精选
bank/         审过的高质量题
```
...
从 `categories/` 精选：信息密度高、无强引导性、表述清晰、与已有题不重复。审阅通过后可工程化为 JSON 迁入 `bank/archive/` 或 `bank/inference/`。
